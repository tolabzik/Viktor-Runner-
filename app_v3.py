...
if b"VIKTOR RUNNER 3.1" not in GAME_JS:
    raise RuntimeError("Invalid VIKTOR RUNNER 3.1 bundle")
for token in (b"function generateChunk", b"supportBoss", b"function cycleLocation", b"boot();"):
    if token not in GAME_JS:
        raise RuntimeError(f"VIKTOR RUNNER 3.1 bundle is incomplete: {token!r}")
